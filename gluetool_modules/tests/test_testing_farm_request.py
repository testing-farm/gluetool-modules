...
ASSETS_DIR = os.path.join('gluetool_modules', 'tests', 'assets', 'testing_farm')
...
def fixture_module():
    api = gluetool_modules.testing_farm.testing_farm_request.TestingFarmAPI
    api.get_request = lambda _, id, key: REQUESTS[key][id]
    api.put_request = lambda _, id, payload: PUT_REQUESTS.update({id: payload})
    module = create_module(gluetool_modules.testing_farm.testing_farm_request.TestingFarmRequestModule)[1]
...
def fixture_module_request():
    module_request = create_module(gluetool_modules.testing_farm.testing_farm_request.TestingFarmRequest)[1]
    return module_request


@pytest.fixture(name='requests_mock')
def fixture_requests_mock():
    gluetool_modules.testing_farm.testing_farm_request.requests = requests_mock


@pytest.fixture(name='module_api')
def fixture_module_api(requests_mock):
    module_api = create_module(gluetool_modules.testing_farm.testing_farm_request.TestingFarmAPI, add_shared=False)[1]
    return module_api
...
def fixture_request1(module):
    ...
    module._tf_api = gluetool_modules.testing_farm.testing_farm_request.TestingFarmAPI(module, module.option('api-url'))
    module._tf_request = gluetool_modules.testing_farm.testing_farm_request.TestingFarmRequest(module)
...
def test_testing_farm_request(module, request1):
    ...
    assert isinstance(request, gluetool_modules.testing_farm.testing_farm_request.TestingFarmRequest)
