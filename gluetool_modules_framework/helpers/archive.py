...
class Archive(gluetool.Module):
    ...
    The ``SOURCE`` is a path to the directory which should be archived. It is glob.
    The ``EXCLUDE`` is a list of patterns to exclude files from the archive. The entries are regexes.
    The ``DESTINATION`` is a path to the destination directory.
    The ``PERMISSIONS`` is a permissions for the destination directory.
    The ``STAGE`` is a stage when the archive should be performed. Look at ARCHIVE_STAGES for available stages.
