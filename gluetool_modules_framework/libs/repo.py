...
def create_repo(sut_installation: SUTInstallation, repo_name: str, repo_path: str) -> None:
    ...
    sut_installation.add_step('Create repository', f'dnf -y install createrepo; createrepo {shlex.quote(repo_path)}', ignore_exception=True)
    sut_installation.add_step('Add repository', f'echo -e "{repo_str.getvalue()}" > /etc/yum.repos.d/{shlex.quote(repo_name)}.repo', ignore_exception=True)
