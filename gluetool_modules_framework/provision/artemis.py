...
class ArtemisGuest(NetworkedGuest):
    ...
    def event_log_error(self) -> Optional[str]:
        if not os.path.exists(self.event_log_path):
            self.warning('Skipping event log analysis, no event log found for the guest')
            return None
...
class ArtemisProvisioner(gluetool.Module):
        ...
        ('Error options', {
            ...
            'error-template-file': {
                'help': """
                        ...
                        The template has access to last Artemis guest log events specified by the ``error-events`` option.
                        The events are accessible under the ``event`` dict variable in the template.
                        """,
                'metavar': 'JINJA_TEMPLATTE',
                'type': str
            },
            'analyze-event-log': {
                'help': """
    ...
    def guest_event_log(self) -> Optional[str]:
        """
        ...
        if analyzed_log.startswith('http'):
            ...
            return load_yaml(gluetool.utils.normalize_option_path(analyzed_log))
    ...
    def sanity(self) -> None:
        ...
        if self.option('provision') and self.option('analyze-guest-event-log'):
            raise GlueError('Option --provision and --analyze-event-log are mutually exclusive.')

        if self.option('analyze-guest-event-log') and not self.option('error-template-file'):
    ...
    def event_log_error(self, events: Any) -> Optional[str]:
        """
        Analyze guest event log and return a resonable error rendered using the error template.
        """
        if not events:
        ...
        # search for last error and pool-resources-requested events
