...
class TestScheduleTMT(Module):
    ...
    def _is_plan_empty(self,
        ...
        # TODO: save this into workdir, but the dir wasn't created yet, use ID of the environment to avoid collisions
        self._save_output(tmt_output, f'discover-{id(testing_environment)}{plan.replace("/", "-")}.txt')
    ...
    def create_test_schedule(
        ...
        for tec in testing_environment_constraints:
            ...
            for plan in plans:
                ...
                if self._is_plan_empty(plan, tmt_env_file, repodir, context_files, tec):
    ...
    def serialize_test_schedule_entry_results(self, schedule_entry: TestScheduleEntry, test_suite: TestSuite) -> None:
        ...
        if schedule_entry.result == TestScheduleResult.SKIPPED:
            tmt_discover_log_href = artifacts_location(
                self,
                f'discover-{id(schedule_entry.testing_environment)}{schedule_entry.plan.replace("/", "-")}.txt',
