...
from gluetool.utils import Command, load_yaml, new_xml_element, dict_update, from_yaml, \
    cached_property, normalize_shell_option
...
class TestScheduleTMT(Module):
    ...
    @cached_property
    def _tmt_context_from_testing_farm(self):
        # type: () -> List[str]
    ...
    def _plans_from_dist_git(self, repodir, context_files, filter=None):
        ...
        if self._tmt_context_from_testing_farm:
            command.extend(self._tmt_context_from_testing_farm)
        ...
        for tec in testing_environment_constraints:
            ...
            for plan in plans:
                ...
                schedule_entry.testing_environment = TestingEnvironment(
                    ...
                    pool=tec.pool,
                    hardware=self.hardware_from_request() or self.hardware_from_tmt(repodir, plan, context_files)
                )
