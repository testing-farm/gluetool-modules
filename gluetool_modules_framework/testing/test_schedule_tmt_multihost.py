...
class TestArtifact:
    ...
    # We might attempt to store a single artifact under various names into results, so store them first into a set and
    # allow only one occurrence of each `path`.
    def __hash__(self) -> int:
        return hash(self.path)

    def __eq__(self, other: Any) -> bool:
        return isinstance(other, TestArtifact) and self.path == other.path
