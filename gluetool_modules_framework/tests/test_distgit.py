...
def fixture_module():
    return create_module(gluetool_modules_framework.infrastructure.distgit.DistGit)[1]
...
def test_artifact(monkeypatch, module):
    ...
    monkeypatch.setattr(gluetool_modules_framework.infrastructure.distgit, 'PatternMap', pattern_map_mock)
    monkeypatch.setattr(gluetool_modules_framework.infrastructure.distgit, 'render_template', lambda a: 'a-value')
...
def test_artifact_override(monkeypatch, module):
    ...
    monkeypatch.setattr(gluetool_modules_framework.infrastructure.distgit, 'render_template', lambda a: a)
