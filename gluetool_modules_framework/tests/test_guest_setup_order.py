...
def fixture_module(monkeypatch):
    ...
    patch_shared(monkeypatch, module, {
        'setup_guest': None,
        'guest-setup-fedora-koji-build': Ok('setup-guest-fedora-koji-build'),
        'guest-setup-fedora-copr-build': Ok('setup-guest-fedora-copr-build'),
        'setup-guest-redhat-brew-build': Ok('setup-guest-redhat-brew-build'),
        'guest-setup-repository': Ok('setup-guest-repository'),
        'setup-guest-repository-file': Ok('setup-guest-repository-file'),
    })
...
def test_guest_setup_error(monkeypatch, module, log):

    setup_error = Error('setup-guest-fedora-koji-build')

    patch_shared(monkeypatch, module, {
        'guest-setup-fedora-koji-build': setup_error,
    })

    execute_mock = MagicMock(return_value=MagicMock(stdout='', stderr=''))
    guest = mock_guest(execute_mock)

    output = module.setup_guest(guest, stage=GuestSetupStage.ARTIFACT_INSTALLATION,
                                log_dirpath=str('tmpdir'))

    assert output == setup_error
