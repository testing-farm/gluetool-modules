...
import gluetool
import gluetool_modules_framework.infrastructure.mbs
from gluetool_modules_framework.infrastructure.mbs import MBS, TaskArches, nsvc_from_nvr
...
def fixture_get(mbs_module_info, monkeypatch):
    ...
    monkeypatch.setattr(gluetool_modules_framework.infrastructure.mbs.requests, 'get', dummy_request)


def test_loadable(module):
    check_loadable(module.glue, 'gluetool_modules_framework/infrastructure/mbs.py', 'MBS')
...
def test_invalid_url(module, monkeypatch):
    monkeypatch.setattr(
        gluetool_modules_framework.infrastructure.mbs.requests, 'get', MagicMock(side_effect=Exception('fake'))
    )
...
def test_mbs_task_invalid_init(module):
    with pytest.raises(gluetool.GlueError, match='module must be initialized only from one of build_id, nsvc or nvr'):
        gluetool_modules_framework.infrastructure.mbs.MBSTask(module, build_id=2178, nvr='nvr')
