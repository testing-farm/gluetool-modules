...
def fixture_module(monkeypatch, tmpdir):
    ...
    monkeypatch.setattr(
        gluetool_modules_framework.static_analysis.rpminspect.rpminspect.tempfile,
        "mkdtemp",
        MagicMock(return_value=str(workdir))
    )

    module = create_module(gluetool_modules_framework.static_analysis.rpminspect.rpminspect.CIRpminspect)[1]
...
def test_loadable(module):
    check_loadable(module.glue, 'gluetool_modules_framework/static_analysis/rpminspect/rpminspect.py', 'CIRpminspect')
...
def test_run_rpminspect(module, monkeypatch):
    ...
    monkeypatch.setattr(gluetool_modules_framework.static_analysis.rpminspect.rpminspect, 'Command', mock_command)
...
def test_execute(module, monkeypatch, log):
    ...
    monkeypatch.setattr(gluetool.utils.Command, 'run', mock_command_run)
    monkeypatch.setattr(
        gluetool_modules_framework.static_analysis.rpminspect.rpminspect, 'load_json', MagicMock(return_value={})
    )
    ...
    assert log.records[-1].message == """result:
{
    ...
    "result_class": "gluetool_modules_framework.static_analysis.rpminspect.rpminspect.RpminspectSkippedTestResult",
...
def test_sanity(module, monkeypatch):
    ...
    monkeypatch.setattr(gluetool_modules_framework.static_analysis.rpminspect.rpminspect,
