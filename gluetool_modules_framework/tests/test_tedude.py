...
import gluetool
import gluetool_modules_framework.testing.tedude  # importing of a module that will be tested
...
def fixture_module():
    ...
    return create_module(gluetool_modules_framework.testing.tedude.TeDuDe)[1]


@pytest.fixture(name='module_with_results')
def fixture_module_with_results():
    # the function returns glue and module instances. We are interested in the module only.
    ci, module = create_module(gluetool_modules_framework.testing.tedude.TeDuDe)
    module_results = gluetool_modules_framework.testing.testing_results.TestingResults(ci, "dummy_results_module")
...
def fixture_result(module):
    # the function returns glue and module instances. We are interested in the module only.
    return create_module(gluetool_modules_framework.testing.tedude.TeDuDeTestResult, add_shared=False)[1]


def test_loadable(module):
    check_loadable(module.glue, 'gluetool_modules_framework/testing/tedude.py', 'TeDuDe')
    ...
    assert log.records[-1].message == """result:
{
    ...
    "result_class": "gluetool_modules_framework.testing.tedude.TeDuDeTestResult",
    "test_type": "tedude",
    "urls": {}
}"""
