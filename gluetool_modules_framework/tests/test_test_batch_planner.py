...
import gluetool_modules_framework.dispatchers.test_batch_planner

from . import create_module, check_loadable, patch_shared

ASSETS_DIR = os.path.join('gluetool_modules_framework', 'tests', 'assets', 'test_batch_planner')
...
def fixture_module(monkeypatch):
    module = create_module(gluetool_modules_framework.dispatchers.test_batch_planner.TestBatchPlanner)[1]
    ...
    rules_engine = gluetool_modules_framework.helpers.rules_engine.RulesEngine(module.glue, 'rules-engine')
...
def test_sanity(module):
    ...
    assert isinstance(module, gluetool_modules_framework.dispatchers.test_batch_planner.TestBatchPlanner)


def test_loadable(module):
    check_loadable(module.glue, 'gluetool_modules_framework/dispatchers/test_batch_planner.py', 'TestBatchPlanner')
