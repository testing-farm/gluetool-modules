...
cd some-download-path && curl -sL --retry 5 --remote-name-all -w "%{url_effective} %{filename_effective}\\n" http://copr/project/one.rpm http://copr/project/two.rpm http://copr/project/one.src.rpm http://copr/project/two.src.rpm | awk -v pkglist="pkglist" \'{print "Downloaded:", $1; print $2 >> pkglist}\'''',
...
def test_tmt_output_koji(module, module_dist_git, guest, monkeypatch, tmpdir, clone_url, expected_clone_url):
    ...
    with open(os.path.join(tmpdir, 'artifact-installation-guest0', INSTALL_COMMANDS_FILE)) as f:
        assert f.read() == '\n'.join([
            ...
            r'''mkdir -pv some-download-path; cat rpms-list-* | xargs bash -c "cp -t some-download-path \$1 && echo $(basename \$1) >> some-download-path/pkglist" --''',
            *generate_createrepo_cmds(repo_name='test-artifacts', repo_path='some-download-path'),
